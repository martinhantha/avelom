...
3. **Secrets** in Coolify setzen (nicht im Git):
   ...
   - Push (FCM): `FIREBASE_SERVICE_ACCOUNT` = kompletter Inhalt der Dienstkonto-JSON (eine Zeile / Multiline-Secret). Nicht die `google-services.json`. Danach `prisma migrate deploy` (Tabelle `DevicePushToken`).
